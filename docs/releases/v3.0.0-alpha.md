...
- **Release Studio**, a committed-revision manufacturing-release flow with
  canonical dossiers, policy evaluation, signed attestations, offline
  verification, and a separate release library. Its operating guides are in
  [the Release Studio documentation](../release-studio/README.md).

> **Screenshot note:** The screenshots in this post are 1920×1080 captures from
> the local Docker deployment. Workspace and project navigation rails, along
> with secondary viewer controls where practical, are minimized so the primary
> evidence remains easy to inspect. The stable image URLs below point to the
> release assets on `main`.
