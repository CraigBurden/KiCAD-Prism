...
DIRTY=false
if ! git -C "${ECAD_VIEWER_DIR}" diff --quiet HEAD -- packages/ecad-viewer-app/src packages/kicad-parser/src packages/ecad-renderer/src; then
  DIRTY=true
fi
if [[ -n "$(git -C "${ECAD_VIEWER_DIR}" status --porcelain --untracked-files=all -- packages/ecad-viewer-app/src packages/kicad-parser/src packages/ecad-renderer/src)" ]]; then
  DIRTY=true
fi
...
RENDERER_SHA="$(shasum -a 256 "${PUBLIC_DIR}/ecad-renderer.js" | awk '{print $1}')"
BUILD_VERSION="prism-native-document-diff-v1"
PATCH_SHA="$(git -C "${ECAD_VIEWER_DIR}" diff --binary HEAD -- packages/ecad-viewer-app/src packages/kicad-parser/src packages/ecad-renderer/src | shasum -a 256 | awk '{print $1}')"
...
# ecad-viewer.js and prism-semantic-viewer.js are <script> tags in
# index.html, so their cache key lives there. The renderer is imported on
# demand -- and by the panel, which has its own HTML -- so its versioned URL is
# emitted as a module both Vite entry points can import instead.
cat > "${PRISM_ROOT}/frontend/src/lib/ecad-renderer-build.ts" <<EOF
...
export const ECAD_RENDERER_URL = "/ecad-renderer.js?v=${RENDERER_SHA}";
EOF
...
cat > "${PUBLIC_DIR}/ecad-viewer.manifest.json" <<EOF
{
  ...
  "artifacts": {
    ...
    "ecad-renderer.js": "sha256:${RENDERER_SHA}"
  }
}
EOF
