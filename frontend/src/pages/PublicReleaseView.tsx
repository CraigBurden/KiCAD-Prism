...
export function PublicReleaseView() {
    ...
    useEffect(() => {
        ...
        load();
        // A revoked or expired share must disappear without a full reload.
        ...
        return (
            <main className="grid min-h-screen place-items-center bg-background p-6 text-foreground">
                <Card className="max-w-lg">
                    <CardHeader><CardTitle>Release unavailable</CardTitle></CardHeader>
        ...
        <main className="min-h-screen bg-background px-5 py-10 text-foreground">
            <div className="mx-auto max-w-5xl space-y-6">
                <header className="flex flex-wrap items-start justify-between gap-4 border-b pb-6">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-muted-foreground">
                            Prism Release Studio
                        </p>
                        <h1 className="mt-2 text-3xl font-semibold tracking-tight">
                            {release.release_label}
                        </h1>
                        <p className="mt-1 text-sm text-muted-foreground">
                            {release.document_number || "No document number"} · revision{" "}
                            {release.revision || "—"}
                        </p>
                        <p className="mt-1 text-xs text-muted-foreground">
                            {formatExpiry(data.expires_at)}
                            {release.created_at
                                ? ` · released ${new Date(release.created_at).toLocaleString()}`
                                : ""}
                        </p>
                    </div>
                    <Badge
                        variant={data.verification.ok ? "secondary" : "destructive"}
                        className="gap-1.5"
                    >
                ...
                <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    {(
                        [
                            ["Commit", short(release.commit_sha)],
                            ["Variant", release.variant || "default"],
                            ["Signing key", short(release.signing_key_id)],
                ...
                <Card>
                    <CardHeader className="flex-row items-center justify-between">
                        <CardTitle className="text-base">Released files</CardTitle>
                        <Button asChild size="sm">
                            <a href={`${apiBase}/archive`}>
                    ...
                    <CardContent className="divide-y p-0">
                        {data.members.map((member) => (
                            <a
                                key={member.path}
                                href={`${apiBase}/members/${member.path.split("/").map(encodeURIComponent).join("/")}`}
                                className="flex items-center justify-between gap-4 px-6 py-3 text-sm hover:bg-muted/60"
                            >
                                <span>
                                    <span className="font-medium">{member.path}</span>
                                    <span className="ml-2 text-xs text-muted-foreground">
                                        {member.domains.join(", ")}
                                    </span>
                                </span>
                                <span className="font-mono text-xs text-muted-foreground">
                                    {short(member.released_digest)}
                                </span>
                            </a>
                        ))}
                    </CardContent>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2">
                        {data.verification.checks.map((check, index) => (
                            <div key={`${index}-${check.message}`} className="flex gap-2 text-sm">
                                <CheckCircle2
                                    className={`mt-0.5 h-4 w-4 shrink-0 ${
                                        check.ok ? "text-emerald-500" : "text-destructive"
                                    }`}
                                />
